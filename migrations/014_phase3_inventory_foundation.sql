...
-- Current live-series migration: 014 (Phase 1/2 migrations already exist in the database).
...
INSERT INTO permissions (code, description) VALUES
 ...
 ('inventory.rebuild', 'Rebuild inventory balance projections')
...
BEGIN
  FOREACH t IN ARRAY ARRAY['idempotency_keys','inventory_balances','inventory_transactions','inventory_adjustments','inventory_adjustment_lines','stock_counts','stock_count_lines','inventory_transfers','inventory_transfer_lines'] LOOP
    ...
    EXECUTE format('CREATE POLICY %I ON %I USING (tenant_id = NULLIF(current_setting(''app.tenant_id'', true), '''')::uuid) WITH CHECK (tenant_id = NULLIF(current_setting(''app.tenant_id'', true), '''')::uuid)', t || '_tenant_isolation', t);
  END LOOP;
END $$;
...
-- Immutable ledger hardening.
REVOKE UPDATE, DELETE ON inventory_transactions FROM lexa_app;
REVOKE UPDATE, DELETE ON inventory_transactions FROM PUBLIC;
CREATE OR REPLACE FUNCTION lexa_prevent_inventory_ledger_mutation()
...
FOR EACH ROW EXECUTE FUNCTION lexa_prevent_inventory_ledger_mutation();
