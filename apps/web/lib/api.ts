...
export type CustomerCredit = { id:string; customer_party_id:string; customer_name:string; source_return_id:string; original_amount:string; allocated_amount:string; balance:string; status:string; created_at:string; updated_at:string };
...
export function getCustomerCredits(){return getJson<CustomerCredit[]>('/api/v1/commerce/customer-credits');}
export function allocateCustomerCredit(id:string,body:{receivable_id:string;amount:string}){return getJson<CustomerCredit>(`/api/v1/commerce/customer-credits/${id}/allocate`,{method:'POST',headers:{'Content-Type':'application/json','Idempotency-Key':key()},body:JSON.stringify(body)});}
...
export function setReconciliationActual(id:string,body:{actual_amount:string;notes?:string}){return getJson<ReconciliationLine>(`/api/v1/commerce/reconciliation-lines/${id}/actual`,{method:'POST',headers:{'Content-Type':'application/json','Idempotency-Key':key()},body:JSON.stringify(body)});}
export function resolveReconciliationLine(id:string,body:{status:'RESOLVED'|'ACKNOWLEDGED';notes?:string}){return getJson<ReconciliationLine>(`/api/v1/commerce/reconciliation-lines/${id}/resolve`,{method:'POST',headers:{'Content-Type':'application/json','Idempotency-Key':key()},body:JSON.stringify(body)});}
