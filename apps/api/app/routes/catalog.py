...
class ProductIn(BaseModel):
    name: str; category_id: UUID; brand_id: UUID | None = None; description: str | None = None; product_type: str = "STOCKED"; has_variants: bool = True; tax_category_id: UUID | None = None; metadata: dict[str, Any] = Field(default_factory=dict)
...
class ProductPriceIn(BaseModel):
    price_list_id: UUID; variant_id: UUID; unit_price: Decimal = Field(ge=0); minimum_quantity: Decimal = Field(default=Decimal("1"), gt=0); effective_from: datetime; effective_to: datetime | None = None
class ProductPriceOut(ORM):
    id: UUID; tenant_id: UUID; price_list_id: UUID; variant_id: UUID; unit_price: Decimal; minimum_quantity: Decimal; effective_from: datetime; effective_to: datetime | None
...
def create_price(body: ProductPriceIn, ctx=Depends(require_permission("catalog.manage"))):
    ...
    overlap = db.scalar(select(ProductPrice.id).where(
        ...
        ProductPrice.price_list_id == body.price_list_id,
        ProductPrice.variant_id == body.variant_id,
        ProductPrice.minimum_quantity == body.minimum_quantity,
