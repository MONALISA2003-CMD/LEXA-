  ...
  return (
    <main className="app-shell">
      <aside className={`sidebar ${mobileMenuOpen ? "open" : ""}`}>
        ...
        <a href="/kernel" className="sidebar-item" style={{textDecoration:"none"}}><span className="sidebar-icon">◆</span><span className="sidebar-text"><b>Business Kernel</b><small>Universal business objects</small></span></a><nav className="sidebar-nav" aria-label="Workspace navigation">{modules.map(item => <button key={item.name} className={`sidebar-item ${active === item.name ? "active" : ""}`} onClick={() => { setActive(item.name); setMobileMenuOpen(false); }}><span className="sidebar-icon"><NavIcon name={item.name} /></span><span className="sidebar-text"><b>{item.name}</b><small>{item.desc}</small></span></button>)}</nav>
